...
function makeUsername(nome: string, cognome: string): string {
...
async function createSession(db: D1Database, user: SessionUser): Promise<string> {
  ...
  await db
    ...
    .bind(crypto.randomUUID(), user.id === "admin" ? null : user.id, user.role, tokenHash, expiresAt)
    .run();
  return token;
}

async function getSessionUser(db: D1Database, token: string): Promise<SessionUser | null> {
  const row = await db
    .prepare(
      `SELECT s.role, s.user_id, s.expires_at, u.nome, u.cognome, u.username
...
async function requireAdmin(c: any): Promise<SessionUser | Response> {
...
app.post("/api/login", async (c) => {
  ...
  const row = await c.env.DB
    .prepare("SELECT id, nome, cognome, username, password_hash, ruolo FROM users WHERE lower(username) = ? LIMIT 1")
    .bind(username)
    .first<{
      id: number; nome: string; cognome: string; username: string; password_hash: string; ruolo: "admin" | "user";
    }>();

  if (!row || row.ruolo !== "user" || !(await verifyPassword(password, row.password_hash))) {
...
app.post("/api/admin/users", async (c) => {
  ...
  const body = await c.req.json<{ nome?: string; cognome?: string; email?: string; password?: string }>();
  const nome = body.nome?.trim();
  const cognome = body.cognome?.trim();
  const password = body.password?.trim() || cognome || "";
  if (!nome || !cognome) return c.json({ success: false, error: "Nome e cognome sono obbligatori" }, 400);
  if (password.length < 3) return c.json({ success: false, error: "La password deve contenere almeno 3 caratteri" }, 400);
  ...
  if (existing) return c.json({ success: false, error: `Esiste già un collaboratore con username ${username}` }, 409);

  const passwordHash = await hashPassword(password);
  const result = await c.env.DB
    .prepare("INSERT INTO users (nome, cognome, username, email, password_hash, ruolo) VALUES (?, ?, ?, ?, ?, 'user')")
    .bind(nome, cognome, username, body.email?.trim() || null, passwordHash)
    .run();

  return c.json({
    success: true,
    user: { id: result.meta.last_row_id, nome, cognome, username, email: body.email?.trim() || null, role: "user" },
  }, 201);
});
...
app.get("/api/admin/users", async (c) => {
  ...
  const users = await c.env.DB
    .prepare("SELECT id, nome, cognome, username, email, ruolo, created_at FROM users WHERE ruolo = 'user' ORDER BY cognome, nome")
    .all();
...
app.get("/api/admin/users/:id", async (c) => {
  ...
  const user = await c.env.DB
    .prepare("SELECT id, nome, cognome, username, email, ruolo, created_at FROM users WHERE id = ?")
    .bind(id)
    .first();
...
app.patch("/api/admin/users/:id/password", async (c) => {
  ...
  if (!Number.isInteger(id) || password.length < 3) return c.json({ success: false, error: "ID o password non validi" }, 400);
  const result = await c.env.DB
    .prepare("UPDATE users SET password_hash = ? WHERE id = ? AND ruolo = 'user'")
